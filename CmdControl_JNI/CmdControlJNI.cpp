...
static const char* XSY_CMDCONTROL_JNI_CLASS_PATH_NAME =
		"com/xinshiyun/utils/CmdControlJNI";
...
JNICmdControlListener::JNICmdControlListener(JNIEnv* env, jobject thiz,
	...
	if (clazz == NULL) {
		LOGE("Can't find com/xinshiyun/utils/CmdControlJNI");
...
void JNICmdControlListener::notify(int what, int ext1, int ext2) {
	...
	LOGD("what = 0x%x, ext1 = 0x%x, ext2 = 0x%x",what,ext1,ext2);
	env->CallStaticVoidMethod(mClass, fields.postEvent, mObject, what, ext1,ext2);
...
static void Xsy_CmdControl_native_init(JNIEnv *env) {
	...
	fields.postEvent = env->GetStaticMethodID(clazz, "postEventFromNative","(Ljava/lang/Object;III)V");
...
static jint Xsy_CmdControl_onExecuteCmd(JNIEnv *env,
	...
	if (serCom != NULL) {
		if(cmd != NULL){
			...
			result = serCom->onExcuteCmd(command);
...
static int register_android_cmd_control(JNIEnv *env) {
	LOGD("register_android_display_control()");
